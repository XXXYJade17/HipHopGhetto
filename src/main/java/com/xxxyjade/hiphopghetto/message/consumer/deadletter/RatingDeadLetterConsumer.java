...
@Component
@Slf4j
public class RatingDeadLetterConsumer {
    ...
    // 监听收藏死信队列
    @RabbitListener(queues = MessageQueue.RATING_DEAD_QUEUE)
    public void handleDeadLetterMessage(Message<Rating> message) {
        ...
        if (retryCount.incrementAndGet() <= 3) {
            ...
            rabbitTemplate.convertAndSend(MessageQueue.RATING_QUEUE, message);
        } else {
