...
@Component
@Slf4j
public class CollectionCountIncreaseHandler implements MessageStrategy<Collection> {
    ...
    public void handle(Message<Collection> message) {
        try {
            ...
            switch (collection.getTargetType()) {
                case ResourceType.ALBUM -> albumService.increaseCollectionCount(targetId);
                case ResourceType.SONG -> songService.increaseCollectionCount(targetId);
...
//            throw new HipHopGhettoFrameException("消息处理失败");
    ...
    public String getMessageType() {
        return MessageType.COLLECTION_COUNT_INCREASE;
    }
}