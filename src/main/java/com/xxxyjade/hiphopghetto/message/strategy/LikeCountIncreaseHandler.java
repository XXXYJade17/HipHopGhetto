...
@Component
@Slf4j
public class LikeCountIncreaseHandler implements MessageStrategy<Like> {
    ...
    public void handle(Message<Like> message) {
        try {
            ...
            switch (like.getTargetType()) {
                case ResourceType.COMMENT -> commentService.increaseLikeCount(targetId);
                case ResourceType.TOPIC -> topicService.increaseLikeCount(targetId);
...
//            throw new HipHopGhettoFrameException("消息处理失败");
    ...
    public String getMessageType() {
        return MessageType.LIKE_COUNT_INCREASE;
    }
}